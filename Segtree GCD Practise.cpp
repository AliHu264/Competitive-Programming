...
int findGcd(int ss, int se, int qs, int qe, int si)
{
    ...
    if (qs<=ss && qe>=se)
        return st[si];
    int mid = ss+(se-ss)/2;
    return __gcd(findGcd(ss, mid, qs, qe, si*2),
                 findGcd(mid+1, se, qs, qe, si*2+1));
}
...
int constructST(int arr[], int ss, int se, int si)
{
    ...
    int mid = ss+(se-ss)/2;
    st[si] = __gcd(constructST(arr, ss, mid, si*2),
                   constructST(arr, mid+1, se, si*2+1));
    return st[si];
}
...
vector<int> constructSegmentTree(int arr[], int n)
{
    int height = (int)(ceil(log2(n)));
    int size = 2*(int)pow(2, height)-1;
    st.resize(size);
    constructST(arr, 0, n-1, 1);
    return st;
}
